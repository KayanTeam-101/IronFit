...
const MEAL_NAMES_AR: Record<string, string> = {
  Breakfast: "الفطور",
  Lunch: "الغداء",
...
const Meal = React.memo(
  ({
    ...
    const getVitamins: string[] = useMemo(
      () => dietPlan?.[mealName]?.[1]?.[4] || [],
      [dietPlan, mealName]
    );
    ...
    const consumedNutrition = useMemo(() => {
      ...
      return { calories: cal, protein: prot, fat, carb };
    }, [eatenDishes, foodInfoList, mealName, dietPlan]);
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-2xl dark:text-white text-gray-800 flex items-center gap-2">
            ...
            <span className="text-2xl">{MEAL_ICONS[mealName]}</span>
          </h2>
          {totalPlanned > 0 && (
            <div className="relative w-10 h-10">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                <circle cx="18" cy="18" r="15.5" fill="none" stroke="rgba(0,0,0,0.05)" strokeWidth="3" />
                <circle
                  cx="18" cy="18" r="15.5" fill="none"
                  stroke="url(#mealGradient)" strokeWidth="3" strokeLinecap="round"
        ...
        {plannedDishes.length > 0 ? (
          ...
          <div className="text-center text-gray-400 py-4">لا توجد وجبات مخططة لهذه الوجبة</div>
        )}
