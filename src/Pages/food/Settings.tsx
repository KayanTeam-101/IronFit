...
const Settings = () => {
    const SuringToResetADiet = () =>{
        const Sure = confirm("هل أنت متأكد من إعادة تعيين النظام الغذائي؟ سيتم حذف جميع بياناتك الحالية.");
        if (Sure) {
            localStorage.removeItem("Diet");
            localStorage.removeItem("FoodInfo_s");
  ...
  return (
    <div className="fixed show-first top-0 left-0 z-50 w-screen h-lvh flex justify-center items-center bg-black/5 backdrop-blur-sm ">
      <div className="w-10/12 min-h-62 p-5 bg-white dark:bg-black/20 dark:border-2 dark:border-gray-600/20 rounded-2xl">
