...
const TodayTask = () => {
  ...
  useEffect(() => {
    giveHealthAdvice();
    SetProtine(Number(localStorage.getItem("totalProtine")));
    SetCarb(Number(localStorage.getItem("totalCarb")));}
,[])
  const toggleRestDay = () => {
    const stored = JSON.parse(localStorage.getItem('none-exercise') || '[]');
    ...
    return (
      <div className="w-full mt-1">
        ...
        <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400 mt-0.5">
          <span className='p-1'>{current}{target < 500 ? "جم" : ""}</span>
  ...
  return (
    <div className="bg-white dark:bg-[#222]/20 dark:border-2 dark:border-gray-600/20 rounded-4xl border border-amber-100 p-4 shadow-sm mb-6.5 z-10">
      ...
      <div className="flex flex-row  gap-2 justify-center">
        ...
        <div className={`${localStorage.getItem("Diet") ? "" : "opacity-10"} flex flex-col items-center max-w-[100px] active:scale-95 transition delay-100 flex-1 p-3 rounded-xl bg-gray-50 shadow  dark:bg-[#222]/50 `}>
          <GiBiceps className="text-3xl text-blue-600 mb-1" />
          <p className="text-[12px] p-1 font-medium dark:text-white">البروتين</p>
          <ProgressBar current={allProtine === 0 ? "???" : allProtine.toFixed(1)} target={Math.round(Number(localStorage.getItem("currentWeight")) * 1.6)} color="bg-blue-600" />
        </div>
        ...
        <div className={`flex flex-col items-center max-w-[100px] active:scale-95 transition delay-100 flex-1 p-3 rounded-xl bg-gray-50 shadow dark:bg-[#222]/50 ${localStorage.getItem("Diet") ? "" : "opacity-10"}`}>
          ...
          <ProgressBar current={allCarb.toFixed(1)} target={50} color="bg-amber-500" />
        </div>
