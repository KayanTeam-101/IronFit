...
const SetNextDay = () => {
  ...
  const navigate = useNavigate();
   ...
   <>
  { SetNextDay() ? (
     <div className='relative min-h-96 w-full p-5 bg-linear-to-b from-sky-500 overflow-hidden dark:from-slate-800/40 dark:to-black/40 dark:border-2 dark:border-gray-600/20  to-blue-500 rounded-3xl border-sky-50 flex flex-col gap-10'>
<div className='flex flex-row items-center justify-between'>
</div>
<div className='relative w-full h-full'>
    <h1 className='text-5xl text-white text-shadow-lg font-bold mb-2 leading-14'>اليوم القادم {SetNextDay()} <div><GiShoulderArmor className='absolute text-9xl scale-200 left-0 top-0 opacity-20'/></div></h1>
<div className='absolute -left-1 -top-2 w-full h-full  '></div>
</div>
    <button 
    onClick={() => navigate('/exercises')}
    className='flex cursor-pointer items-center gap-2 bg-white p-3 shadow-2xl dark:bg-black/20 dark:border-2 dark:border-gray-600/20 dark:text-slate-300 overflow-hidden w-fit rounded-xl text-sky-500 font-bold'>
      الذهاب لصفحة التمارين <FaArrowLeft />
    </button>
    </div>
   ) : (
    <div className='relative min-h-96 w-full p-5 bg-slate-300 overflow-hidden dark:bg-black/20 dark:border-2 dark:border-gray-600/20  rounded-2xl border-gray-200 flex flex-col gap-10'>
<div className='flex flex-row items-center justify-between'>
