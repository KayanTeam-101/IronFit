...
const AnimatedCheck = () => (
  ...
  >
    <circle cx="12" cy="12" r="10" opacity="0" strokeDasharray="63" strokeDashoffset="63">
      <animate attributeName="stroke-dashoffset" from="63" to="0" dur=".7s" fill="freeze" />
      <animate attributeName="opacity" from="0" to="1" dur="1s" fill="freeze" />
    </circle>
    <path d="M8 12 l3 3 5 -5" strokeDasharray="20" strokeDashoffset="20">
      <animate attributeName="stroke-dashoffset" from="20" to="0" dur="1s" begin="0.2s" fill="freeze" />
    </path>
  </svg>
);
...
const Subscription: React.FC = () => {
  ...
  useEffect(() =>{
    async function IsSubscribe(){
      ...
      const getUser = getUsers_.find(item => item.UserName === localStorage.getItem('UserName'));
      if(getUser?.SubscriptionPeriod && getUser?.SubscriptionPeriod > new Date().getTime()){
        localStorage.setItem("SubscriptionPeriod", String(getUser?.SubscriptionPeriod)); 
        localStorage.setItem("foods____", btoa(JSON.stringify({ SubscriptionPeriod: getUser?.SubscriptionPeriod })));
        console.log("done");
    ...
    IsSubscribe();
  },[activeTab, friendId, customDays, copied, activateSuccess, subscribeSuccess, selectedFixedPlan]);
  // Copy user ID
  const handleCopy = () => {
  ...
  const handleActivate =async () => {
    ...
    // If Friend's Code
    const getFriendCode = getUsers_.find(item => item.UserId_ === Number(friendId));   
    const getUser = getUsers_.find(item => item.UserName === localStorage.getItem('UserName'));

    if (getFriendCode?.UserId_ === Number(friendId) && Number(friendId) !== Number(getUser?.UserId_) && getFriendCode.mycodeUsed === false) {
      const CreateASubscriptionPeriod = new Date().getTime() + 3 * 24 * 60 * 60 * 1000;

      // Update data
      updateUserSubscription(getFriendCode.id, CreateASubscriptionPeriod, true);
      updateUserSubscription(String(getUser?.id), CreateASubscriptionPeriod, true);

      // Save to localStorage
      localStorage.setItem("SubscriptionPeriod", String(CreateASubscriptionPeriod)); 
...
setSubscribeSuccess(true);
  ...
  const createRipple = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    ripple.className = "absolute bg-amber-300 rounded-full animate-ripple pointer-events-none";
    ...
    {Number(localStorage.getItem("SubscriptionPeriod")) < new Date().getTime() ? (
      ...
      <div className="relative z-10 max-w-lg mx-auto px-4 py-8 space-y-2 mb-20">
        ...
        <div className="text-center">
          <div className="text-5xl font-bold text-amber-600 dark:text-amber-400 flex justify-center showInTwoSecond"><IoDiamondOutline /></div>
          <p className="dark:text-white mt-1 showInTwoSecond">نظم حياتك و طورها لأنك تستحق </p>
        ...
        <div className="bg-white/50 dark:bg-black/20 dark:border-2 dark:border-gray-600/20 backdrop-blur-xl border border-white/50  shadow-xl  p-2 flex items-center justify-between transition-all hover:shadow-2xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm text-gray-500 dark:text-gray-400">الكود التعريفي</div>
              <div className="text-2xl font-bold text-gray-800 dark:text-white tracking-widest">{userID}</div>
            </div>
          </div>
          <button
            onClick={handleCopy}
          ...
          >
            ...
            {copied && <span className="text-green-600 dark:text-green-400 text-xs mr-1">تم النسخ</span>}
          </button>
        </div>
        ...
        <div className="bg-white/50 dark:bg-black/20 dark:border-2 dark:border-gray-600/20  backdrop-blur-xl border border-white/50  shadow-xl  p-5 transition-all hover:shadow-2xl">
          ...
          {activateSuccess && (
            <p className="text-green-600 dark:text-green-400 text-sm mt-2">تم التفعيل بنجاح!</p>
          )}
        </div>
        ...
        <div className="flex gap-1 p-1 bg-white/50 dark:bg-black/20 dark:border-2 dark:border-gray-600/20 backdrop-blur-xl border border-white/50  shadow-xl">
          {["3", "7", "30", "custom"].map((tab) => (
            <button
              key={tab}
              onClick={() => {setActiveTab(tab);window.navigator.vibrate(70)}}
          ...
          >
            {activeTab === "custom" ? (
              <div className="bg-white/50 dark:bg-black/20 dark:border-2 dark:border-gray-600/20 backdrop-blur-xl border border-white/50  shadow-xl  p-5">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">اختر عدد الأيام (1 – 90)</p>

                <div className="flex flex-col items-center gap-4">
                  {/* SVG ring */}
                  <div className="relative w-48 h-48">
                    <svg width="100%" height="100%" viewBox="0 0 160 160" className="transform -rotate-90">
                      <circle cx="80" cy="80" r={radius} fill="none" stroke="rgba(251,191,36,0.15)" strokeWidth="8" />
                      <circle
                        cx="80"
                        cy="80"
                        r={radius}
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-3xl font-extrabold text-gray-800 dark:text-white">{customDays}</span>
                  ...
                  <input
                    ...
                    max="90"
                    value={customDays}
                    onChange={(e) => {setCustomDays(Number(e.target.value));if(window.navigator){window.navigator.vibrate(30)}}}
                  ...
                  <div className="flex justify-between w-full">
                    ...
                    <span className="text-xl font-bold text-amber-600 dark:text-amber-400">{customPrice} ج.م</span>
                  </div>
              ...
              <div className="space-y-4">
                {FIXED_PLANS.filter(p => p.days === Number(activeTab)).map(plan => (
                  <div
                    key={plan.days}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <span className="text-xl font-bold text-gray-800 dark:text-white">{plan.price}</span>
